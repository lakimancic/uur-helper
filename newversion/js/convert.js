...
// Conversion from arbitary number system to deicmal

document.getElementById('convert1').onclick = () => {
    // Base and Number values
    ...
    // Calculation Starting position ( exponent value )
    ...
    // Changing 2-digits values with letters
    if(base > 10) {
...
// Conversion from decimal to arbitary number system

document.getElementById('convert2').onclick = () => {
    // Variables for Base, Number and Precision of conversion
    ...
    let decimalPart = utils.stabilize(num - Math.floor(num));
    ...
    if(decimalPart) {
        ...
        while(cnt < precision && decimalPart !== 0) {
            let newNum = utils.stabilize(decimalPart * base);
            let intPart = Math.floor(newNum);

            row2.innerHTML += `<span class="px-3">${decimalPart} * ${base} = <span class="redc">${intPart > 9 ? `${String.fromCharCode(65 + intPart - 10)} ← ` : ''}${intPart}</span>.${utils.stabilize((newNum - intPart)*10).toString().replace('.', '')}</span>`;

            digs += (intPart > 9 ? String.fromCharCode(65 + intPart - 10) : (intPart).toString());
            decimalPart = utils.stabilize(newNum - intPart);
            cnt++;
        }
